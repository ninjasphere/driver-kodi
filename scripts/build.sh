...
if [ "$BUILDBOX_BRANCH" = "master" ]; then
	GOOS=linux GOARCH=arm go build -ldflags "-X main.GitCommit ${GIT_COMMIT}${GIT_DIRTY}" -tags release -o ./bin/${BIN_NAME}
else
	GOOS=linux GOARCH=arm go build -ldflags "-X main.GitCommit ${GIT_COMMIT}${GIT_DIRTY}" -o ./bin/${BIN_NAME}
fi
